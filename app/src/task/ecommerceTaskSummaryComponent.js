...
(function () {
	...
	function ecommerceTaskSummaryController($scope, ecommerceTaskApi, ngTableParams, homeSharedService, $location, taskService) {
			...
			return new ngTableParams(
				{
					...
					getData: function ($defer, params) {
						...
						self.getAllActiveTasks(params.page() - 1);
					}
				}
			)
		};
		...
		/**
		 * Fetches all the active tasks from database with pagination.
		 * @param page  selected page number.
		 */
		self.getAllActiveTasks = function(page) {
			ecommerceTaskApi.getAllActiveTasks(
				{
		 ...
		 */
		self.fetchTasksByStatus = function(isFetchAllSelected) {
			self.fetchAll = isFetchAllSelected;
			self.reloadTable(self.tableParams.page(), true);
		}

...
(function () {
	...
	function CasePackImportController(productFactory, PMCommons, $scope,$rootScope, productCasePackApi, userApi,
		...
		self.setDateForProrationDatePicker = function(){
			...
			if(self.currentImport.prorationDate) {
				self.currentProrationDate =
					new Date(self.currentImport.prorationDate);
			} else {
				self.currentProrationDate = null;
			}
		};
		...
		self.setDateForInStoreDatePicker = function(){
			...
			if(self.currentImport.inStoreDate) {
				self.currentInStoreDate =
					new Date(self.currentImport.inStoreDate);
			} else {
				self.currentInStoreDate = null;
			}
		};
		...
		self.setDateForWarehouseFlushDatePicker = function(){
			...
			if(self.currentImport.warehouseFlushDate) {
				self.currentWarehouseFlushDate =
					new Date(self.currentImport.warehouseFlushDate);
			} else {
				self.currentWarehouseFlushDate = null;
			}
		};
		...
		self.setDateForDutyConfirmationDatePicker = function(){
			...
			if(self.currentImport.dutyConfirmationDate) {
				self.currentDutyConfirmationDate =
					new Date(self.currentImport.dutyConfirmationDate);
			} else {
				self.currentDutyConfirmationDate = null;
			}
		};
		...
		self.setDateForFreightConfirmationDatePicker = function(){
			...
			if(self.currentImport.freightConfirmationDate) {
				self.currentFreightConfirmationDate =
					new Date(self.currentImport.freightConfirmationDate);
			} else {
				self.currentFreightConfirmationDate = null;
			}
		};
		...
		self.getUserById = function() {
			if (self.itemMaster.addedUsrId && self.itemMaster.addedUsrId.toString().trim() !== '') {
				userApi.getUserById({userId: self.itemMaster.addedUsrId},
					function (results) {
						self.createUser = results;
						if(self.createUser.displayName === null || self.createUser.displayName === undefined) {
							self.createUser.displayName = self.itemMaster.addedUsrId;
						}
					},//error handle
					self.fetchError);
			}

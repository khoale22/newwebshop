	...
	function ECommerceViewSpecificationsController(eCommerceViewApi, $scope, $timeout, $sce) {
		...
		this.$onChanges = function () {
			...
			if (self.currentTab && self.currentTab.id && self.currentTab.id === 'Dinner_TN') {
				self.getDinnerTonightSpecification();
			}else{
		 ...
		 */
		self.getDinnerTonightSpecification = function () {
			self.isWaitingForResponse = true;
			self.specificationList = null;
			eCommerceViewApi.getDinnerTonightSpecification(
					{
						productId: self.productMaster.prodId,
					...
					function (result) {
						self.isWaitingForResponse = false;
						if(result != null && result.specifications != null && result.specifications.length > 0){
							self.specificationList = result.specifications;
						}
					},self.fetchError);
		};

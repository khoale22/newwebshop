...
(function () {
	...
	function IngredientController($scope, eCommerceViewApi, $sce, permissionsService,$rootScope) {
		...
		/**
		 * The source system id and physical attribute id of dinner tonight source.
		 *
		 * @type {string}
		 */
		self.DINNER_TONIGHT_SOURCE_PHYSICAL = "27_1643";
		...
		/**
		 * The dinner tonight source selected when click Edit text button.
		 *
		 * @type {boolean}
		 */
		self.isDinnerTonightSourceSelected = false;
		...
		this.$onChanges = function () {
			...
			self.isDinnerTonightSourceSelected = false;
			self.isProductMaintenanceSourceSelected = false;
			if (self.isDinnerTonightTab()) {
				self.findIngredientsForDinnerTonight();
			} else {
				self.findIngredients();
			}
		};
		...
		/**
		 * Find the Ingredients for Dinner tonight by product id or upc.
		 */
		self.findIngredientsForDinnerTonight = function () {
			eCommerceViewApi.findIngredientsForDinnerTonight({
					productId: self.productMaster.prodId,
					scanCodeId: self.productMaster.productPrimaryScanCodeId
				},
				self.ingredientResponseSuccess,
		...
		self.editSourceIngredient = function () {
			...
			if (self.isDinnerTonightTab()) {
				eCommerceViewApi.findAllIngredientAttributePrioritiesForDinnerTonight({
						productId: self.productMaster.prodId,
						scanCodeId: self.productMaster.productPrimaryScanCodeId
					},
		...
		self.editTextIngredient = function () {
			...
			if (self.isDinnerTonightTab()) {
				self.isDinnerTonightSourceSelected = true;
			} else {
		 ...
		 * and save Dinner Tonight source information.
		 */
		self.saveIngredientAttributePriorities = function () {
			...
			if (angular.toJson(self.ingredientsTableModal) !== angular.toJson(self.ingredientsTableModalOrig)) {
				angular.forEach(self.ingredientsTableModal, function (ingredient) {
					var currentSourceSystem = ingredient.sourceSystemId + '_' + ingredient.physicalAttributeId;
					if (self.isDinnerTonightTab()) {
						if (self.DINNER_TONIGHT_SOURCE_PHYSICAL === currentSourceSystem) {
		...
		/**
		 * Check if current tab is Dinner tonight or not.
		 *
		 * @returns {*|boolean}
		 */
		self.isDinnerTonightTab = function () {
			return self.currentTab && self.currentTab.id && self.currentTab.id === 'Dinner_TN';
		};
		...
		self.ingredientsModalResponseSuccess = function (results) {
			...
			if (!results || results.length === 0) {
				...
				if (self.isDinnerTonightSourceSelected) {
					self.selectedSourceSystem = self.DINNER_TONIGHT_SOURCE_PHYSICAL;
				} else if (self.isProductMaintenanceSourceSelected) {
		...
		/**
		 * In Dinner Tonight tab, ingredient from DinnerTonight source (27) is editable, otherwise not.
		 * In other tabs, ingredient from Product Maintenance source (4) is editable, otherwise not.
		 */
		self.setEditableForModal = function () {
			angular.forEach(self.ingredientsTableModal, function (ingredient) {
				var currentSourceSystem = ingredient.sourceSystemId + '_' + ingredient.physicalAttributeId;
				if (self.isDinnerTonightTab()) {
					if (currentSourceSystem === self.DINNER_TONIGHT_SOURCE_PHYSICAL) {
						ingredient.isEditable = true;	// source 27 editable
					}
					if (!ingredient.content && currentSourceSystem === self.PRODUCT_MAINTENANCE_SOURCE_PHYSICAL
						&& currentSourceSystem !== self.ingredientsECommerceModal.sourceSystemSelected) {
						ingredient.noneDisplay = true;	// hide source 4
					}
				} else {
					if (currentSourceSystem === self.PRODUCT_MAINTENANCE_SOURCE_PHYSICAL) {
						ingredient.isEditable = true;	// source 4 editable
					}
					if (!ingredient.content && currentSourceSystem === self.DINNER_TONIGHT_SOURCE_PHYSICAL
						&& currentSourceSystem !== self.ingredientsECommerceModal.sourceSystemSelected) {
		...
		self.setEditMode = function(){
			if (!self.isDinnerTonightTab()) {
				...
				self.isEBM = permissionsService.getPermissions('PD_ECOM_03', 'EDIT');
			}else{
				self.isEBM = true;
			}
		};

...
(function () {
	...
	function ECommerceViewController(eCommerceViewApi, $scope, $timeout, $filter, $sce, $rootScope, vocabularyService, productGroupService, $state, appConstants, productSearchService, taskService, $location, permissionsService, customHierarchyService) {
		...
		/**
		 * Constanst DINNER TONIGHT.
		 * @type {string}
		 */
		const DINNER_TONIGHT = 'Dinner_TN';
		/**
		 * sale channel of dinner tonight.
		 * @type {string}
		 */
		const SAL_CHANNEL_DINNER_TONIGHT = '07';
		/**
		...
		/**
		 * Secondary description logical attribute id.
		 *
		 * @type {number}
		 */
		self.LOG_ATTR_ID_SECONDDARY_DES = 1665;
		...
		/**
		 * Dinner Tonight Source.
		 *
		 * @type {number}
		 */
		self.SOURCE_DINNER_TONIGHT = 27;
		...
		self.buildECommerceViewTabBySaleChanel = function (saleChannels) {
			self.domainTabBarDataProvider = [
				...
				{
					displayName: "Dinner Tonight",
					id: DINNER_TONIGHT,
					logo: "images/logo_dinner_tonight.png",
					saleChannel: saleChannels[6],
					hierCntxtCd: "DT"
				}
			];
		};
		...
		self.getECommerceViewInformation = function () {
			eCommerceViewApi.getECommerceViewInformation(
				...
				function (results) {
					...
					if(results.pdpTemplateId!=null){
						self.eCommerceViewDetails.pdpTemplateIdOrg = angular.copy(results.pdpTemplateId);
					}
					//validation
		...
		self.save = function () {
			...
			if (self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
				// Check the data change.
				if (self.isDataChanged(self.SOURCE_DINNER_TONIGHT)) {
					// Validate the data change.
					if (self.validateDataChanged()) {
						// save data.
						self.saveDataForDinnerTonight();
					}
				} else {
					// Show message.
		...
		/**
		 * Save data for dinner to night.
		 */
		self.saveDataForDinnerTonight = function () {
			self.isWaitingForResponse = true;
			eCommerceViewApi.updateECommerceViewForDinnerTonight(self.eCommerceViewDetailsParams,
				//success
				function (results) {
					if (self.isRequestPublishAfterSave) {
						// Execute publish after save success.
		...
		/**
		 * Validate data for dinner tonight.
		 *
		 * @returns {boolean}
		 */
		self.validateDataChanged = function () {
			...
			if (self.isProductFullfilmentChanelsChanged()) {
				if (self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
					self.validateFullfilmentChanelDinner();
				}else{
		...
		/**
		 * Validate fullfilment chanel data for dinner tonight.
		 */
		self.validateFullfilmentChanelDinner = function () {
			var errors = [];
			for (var i = 0; i < self.eCommerceViewDetails.productFullfilmentChanels.length; i++) {
				var check = false;
				if(self.checkEffectDateChanged(self.eCommerceViewDetails.productFullfilmentChanels[i])){
		...
		/**
		 * Change the data change for dinner tonight.
		 *
		 * @returns {boolean}
		 */
		self.isDataChanged = function (sourceSystemId) {
			var isChanged = false;
			...
			// Romance Copy.
			if(sourceSystemId === self.SOURCE_DINNER_TONIGHT) {
				if (self.isNotEqual(self.eCommerceViewDetails.dinnerTonightProductDescription, self.eCommerceViewDetails.dinnerTonightProductDescriptionOrg)) {
					self.eCommerceViewDetailsParams.dinnerTonightProductDescription = self.eCommerceViewDetails.dinnerTonightProductDescription;
					self.eCommerceViewDetailsParams.dinnerTonightProductDescription.logicAttributeId = self.LOG_ATTR_ID_ROMANCE_COPY;
					self.eCommerceViewDetailsParams.dinnerTonightProductDescription.sourceSystemId = sourceSystemId;
					isChanged = true;
				}
				if (self.isNotEqual(self.eCommerceViewDetails.secondaryProductDescription, self.eCommerceViewDetails.secondaryProductDescriptionOrg)) {
					self.eCommerceViewDetailsParams.secondaryProductDescription = self.eCommerceViewDetails.secondaryProductDescription;
					self.eCommerceViewDetailsParams.secondaryProductDescription.logicAttributeId = self.LOG_ATTR_ID_SECONDDARY_DES;
					self.eCommerceViewDetailsParams.secondaryProductDescription.sourceSystemId = sourceSystemId;
					isChanged = true;
				}
				if (self.isNotEqual(self.eCommerceViewDetails.dinnerTonightTags, self.eCommerceViewDetails.dinnerTonightTagsOrg)) {
					self.eCommerceViewDetailsParams.dinnerTonightTags = self.eCommerceViewDetails.dinnerTonightTags;
					self.eCommerceViewDetailsParams.dinnerTonightTags.logicAttributeId = self.LOG_ATTR_ID_TAG;
					self.eCommerceViewDetailsParams.dinnerTonightTags.sourceSystemId = sourceSystemId;
					isChanged = true;
				}
			}else{
				...
				if (self.isNotEqualRomanceCopy(self.eCommerceViewDetails.romanceCopy, self.eCommerceViewDetails.romanceCopyOrg)) {
					...
					if(contentHtmlArray.length > 1) {
						...
						self.eCommerceViewDetailsParams.romanceCopy.content = content;
					}else if(contentHtmlArray.length == 1) {
						self.eCommerceViewDetailsParams.romanceCopy.content = self.encodeHTMLEntity(self.htmlToPlaintext(contentHtml));
					}
				}
			}
			// Brand
			if (self.isNotEqual(self.eCommerceViewDetails.brand, self.eCommerceViewDetails.brandOrg)) {
				...
				if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
					self.eCommerceViewDetailsParams.brand.sourceSystemId = sourceSystemId;
				}else{
					self.eCommerceViewDetailsParams.attributeListChange.push(self.LOG_ATTR_ID_BRAND);
				}
				isChanged = true;
			}
			if (self.isNotEqual(self.eCommerceViewDetails.displayName, self.eCommerceViewDetails.displayNameOrg)) {
				...
				if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
					self.eCommerceViewDetailsParams.displayName.sourceSystemId = sourceSystemId;
				}else{
			...
			if (self.isNotEqual(self.eCommerceViewDetails.size, self.eCommerceViewDetails.sizeOrg)) {
				...
				if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
					self.eCommerceViewDetailsParams.size.sourceSystemId = sourceSystemId;
				}else{
			...
			if (self.isNotEqual(self.eCommerceViewDetails.warnings, self.eCommerceViewDetails.warningsOrg)) {
				...
				if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
					self.eCommerceViewDetailsParams.warnings.sourceSystemId = sourceSystemId;
				}else{
			...
			if (self.isNotEqual(self.eCommerceViewDetails.directions, self.eCommerceViewDetails.directionsOrg)) {
				...
				if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
					self.eCommerceViewDetailsParams.directions.sourceSystemId = sourceSystemId;
				}else{
			...
			if (self.isEditingIngredient === true) {
				if (self.isNotEqual(self.eCommerceViewDetails.ingredients, self.eCommerceViewDetails.ingredientsOrg)) {
					...
					if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
						self.eCommerceViewDetailsParams.ingredients.sourceSystemId = sourceSystemId;
					}else{
			...
			if (self.isProductFullfilmentChanelsChanged()) {
				...
				angular.forEach(self.eCommerceViewDetails.productFullfilmentChanels, function (item) {
					...
					if (oldItem === null) {
						...
						if (!self.isDate1EqualToDate2(newItem.effectDate,
							...
							// Update item
							if(sourceSystemId===self.SOURCE_DINNER_TONIGHT){
								newItem.salesChanelCode = SAL_CHANNEL_DINNER_TONIGHT;
							}
							newItem.effectDate = self.convertDateToStringWithYYYYMMDD(newItem.effectDate);
							newItem.expirationDate = self.convertDateToStringWithYYYYMMDD(newItem.expirationDate);
				...
				if (self.eCommerceViewDetails.productFullfilmentChanelsRemoveList !== null &&
					...
					angular.forEach(self.eCommerceViewDetails.productFullfilmentChanelsRemoveList, function (item) {
						if(sourceSystemId===self.SOURCE_DINNER_TONIGHT) {
							item.salesChanelCode = SAL_CHANNEL_DINNER_TONIGHT;
						}
		...
		self.getSalsChnlCd = function () {
			var salsChnlCd = null;
			if (self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
				salsChnlCd = SAL_CHANNEL_DINNER_TONIGHT;
			} else {
		...
		self.reset = function () {
			...
			self.eCommerceViewDetails.isStartDateErrorMgs = false;
			if(self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
				self.eCommerceViewDetails.dinnerTonightProductDescription = angular.copy(self.eCommerceViewDetails.dinnerTonightProductDescriptionOrg);
				self.eCommerceViewDetails.secondaryProductDescription = angular.copy(self.eCommerceViewDetails.secondaryProductDescriptionOrg);
				self.eCommerceViewDetails.dinnerTonightTags = angular.copy(self.eCommerceViewDetails.dinnerTonightTagsOrg);
			}else{
				...
				if(self.eCommerceViewDetails.pdpTemplateIdOrg!=null && self.eCommerceViewDetails.pdpTemplateIdOrg != undefined){
					...
					self.eCommerceViewDetails.pdpTemplateId = null;
				}
			...
			self.eCommerceViewDetails.specificationDinnerTonightData = angular.copy(self.eCommerceViewDetails.specificationDinnerTonightDataOrg);
		...
		/**
		 * This method will be called when click on yes button of publish confirmation modal to call the publishForDinnerTonight method to publish.
		 */
		self.agreePublish = function () {
			$('#confirmPublishModal').modal("hide");
			if (self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
				// Publish the data for dinner tonight.
				self.publishForDinnerTonight();
			} else {
				// publish for other channel.
				self.publishForHeb();
			}
		};
		/**
		...
		/**
		 * This method is used to publish data for dinner tonight.
		 */
		self.publishForDinnerTonight = function () {
			self.isRequestPublishAfterSave = false;
			// Check the data change.
			if (self.isDataChanged(self.SOURCE_DINNER_TONIGHT)) {
				/* validate data change to save. */
		...
		self.doPublish = function () {
			...
			if (self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
				// Publish nutrient, ingredient, tags and specification for dinner tonight.
				eCommerceViewDetailsParams.salsChnlCd = SAL_CHANNEL_DINNER_TONIGHT;
				eCommerceViewDetailsParams.ingredients = {logicAttributeId: self.LOG_ATTR_ID_INGREDIENT};
				eCommerceViewDetailsParams.nutrient = {logicAttributeId: self.LOG_ATTR_ID_NUTRIENT};
				eCommerceViewDetailsParams.dinnerTonightTags = {logicAttributeId: self.LOG_ATTR_ID_TAG};
				eCommerceViewDetailsParams.specification = {logicAttributeId: self.LOG_ATTR_ID_SPEC};
			} else {
				if(self.currentTab.saleChannel != null && self.currentTab.saleChannel !== undefined){
		...
		self.isValidatePublishData = function () {
			...
			if (self.currentTab && self.currentTab.id && self.currentTab.id === DINNER_TONIGHT) {
				if (self.eCommerceViewDetails.dinnerTonightProductDescription == null ||
					self.eCommerceViewDetails.dinnerTonightProductDescription == undefined ||
					self.isEmpty(self.eCommerceViewDetails.dinnerTonightProductDescription.content)) {
					return false;
				}
			}else{
		 ...
		 * code. Return the group information include master data extension, atribute priority...ect
		 */
		...
		self.saveDataAttributePriorities = function () {
			...
			if(angular.toJson(self.eCommerceViewAttributePriority) != angular.toJson(self.eCommerceViewAttributePriorityOrg)){
				...
				if(self.currentAttributeId == 1666 || self.currentAttributeId == 1664){
					if(self.eCommerceViewAttributePriority != null && self.eCommerceViewAttributePriority.eCommerceViewAttributePriorityDetails != null){
						angular.forEach(self.eCommerceViewAttributePriority.eCommerceViewAttributePriorityDetails, function(eCommerceViewPriorityDetails){
							if(eCommerceViewPriorityDetails != null && eCommerceViewPriorityDetails.sourceEditable === true &&
									eCommerceViewPriorityDetails.content != null && eCommerceViewPriorityDetails.content.content != null){
								if(self.currentAttributeId == 1666){
									...
									if(contentHtmlArray.length > 1) {
										...
										eCommerceViewPriorityDetails.content.content = content;
									}else if(contentHtmlArray.length == 1) {
										eCommerceViewPriorityDetails.content.content = self.encodeHTMLEntity(self.htmlToPlaintext(contentHtml));
									}
								}else{

...
(function () {
	...
	function productDescriptionController($timeout, eCommerceViewApi, $scope, $sce, vocabularyService, vocabularyApi, ngTableParams) {
		 ...
		 * @type {number}
		 */
		self.MAX_LENGTH_DESCRIPTION = 1000000000;
		/**
		...
		/**
		 * Holds the display status of dinner tonight.
		 * @type {boolean} true the current tab is dinner tonight or not.
		 */
		self.isDinnerTonightTab = false;
		/**
		...
		this.$onChanges = function () {
			...
			if(self.currentTab && self.currentTab.id && self.currentTab.id === 'Dinner_TN'){
				// Load dinner tonight.
				self.loadDinnerTonightProductDescription();
				self.loadDinnerTonightTags();
				self.isDinnerTonightTab = true;
			}else{
				self.isDinnerTonightTab = false;
				self.loadProductDescription();
				self.loadTags();
		...
		self.loadProductDescription = function () {
			...
			eCommerceViewApi.findProductDescription(
				...
				function (results) {
					...
					self.eCommerceViewDetails.romanceCopy = self.getDataForDinnerTonight(results);
		...
		/**
		 * Get data when use for dinner tonight tab.
		 */
		self.getDataForDinnerTonight = function(data) {
		...
		/**
		 * SubString by max len.
		 *
		 * @param val the string needs to cut.
		 * @param maxLen the max len.
		 * @returns {*}
		 */
		self.subString = function(val, maxLen){
			if(val == null){
				val = '';
			}
		...
		/**
		 * Load secondary product description.
		 */
		self.loadDinnerTonightProductDescription = function(){
			self.eCommerceViewDetails.dinnerTonightProductDescription = {};
			self.eCommerceViewDetails.dinnerTonightProductDescriptionOrg =  {};
			self.eCommerceViewDetails.secondaryProductDescription =  {};
			self.eCommerceViewDetails.secondaryProductDescriptionOrg =  {};
			eCommerceViewApi.findDinnerTonightProductDescription(
				{
					upc: self.productMaster.productPrimaryScanCodeId,
				},
				//success case
				function (results) {
					self.currentNumberOfResponses++;
					self.eCommerceViewDetails.dinnerTonightProductDescription = results.productDescription;
					self.eCommerceViewDetails.secondaryProductDescription = results.secondaryProductDescription;
					if(results.productDescription != null && results.productDescription.content != null) {
						self.eCommerceViewDetails.dinnerTonightProductDescription.content =results.productDescription.content;
					}
					if(results.secondaryProductDescription != null && results.secondaryProductDescription.content != null) {
						self.eCommerceViewDetails.secondaryProductDescription.content = results.secondaryProductDescription.content;
						//	self.eCommerceViewDetails.dinnerTonightProductDescription.content = self.subString(results.productDescription.content, self.MAX_LENGTH_ROMANCE_COPY_DESCRIPTION);

					}
					self.eCommerceViewDetails.dinnerTonightProductDescriptionOrg = angular.copy(self.eCommerceViewDetails.dinnerTonightProductDescription);
					self.eCommerceViewDetails.secondaryProductDescriptionOrg = angular.copy(self.eCommerceViewDetails.secondaryProductDescription);
					self.hideLoading();
					self.validateProductDescription();
				}, self.fetchError);
		}
		/**
		 * Load dinner tonight tags.
		 */
		self.loadDinnerTonightTags = function(){
			self.eCommerceViewDetails.dinnerTonightTags = {};
			self.eCommerceViewDetails.dinnerTonightTagsOrg = {};
			eCommerceViewApi.findDinnerTonightTags(
				{
					productId: self.productMaster.prodId,
					upc: self.productMaster.productPrimaryScanCodeId,
				},
				...
				function (results) {
					self.currentNumberOfResponses++;
					self.eCommerceViewDetails.dinnerTonightTags = results;
					if(results != null) {
						self.eCommerceViewDetails.dinnerTonightTags.content = results.content;
						self.eCommerceViewDetails.dinnerTonightTagsOrg = angular.copy(self.eCommerceViewDetails.dinnerTonightTags);
					}
					self.hideLoading();
				}, self.fetchError);
		}
		...
		self.showDescriptionEditSource = function () {
			...
			self.parent.getECommerceViewDataSource(self.productMaster.prodId, self.productMaster.productPrimaryScanCodeId, 1666, 0,'Romance Copy', null);
		};
		/**
		...
		/**
		 * Show texr area to edit secondary description.
		 */
		self.showEditSecondaryDescription = function(){
			self.editableSecondaryProductDescription = true;
		}
		/**
		...
		/**
		 * Validate the product description and dinner tonight product description. If it is invalid, then show warning message on the top right panel.
		 */

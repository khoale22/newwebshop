	...
	function ProductInfoController(productInfoApi, permissionsService, productSearchApi, userApi,
		...
		function setSelectedCriticalProduct(){
			for(var index = 0; index < self.criticalProductCodeTable.length; index++){
				if(self.currentProductMasterParams.productMaster.goodsProduct.criticalProductSwitch === self.criticalProductCodeTable[index].id){
		...
		self.findAllSubCommoditiesByPageRequest = function(){
			...
			function (results) {//get 200 rows at first time
				...
				var tempList = tempArray.concat(results.filter(function(i) {
			...
			if(self.checkSubCommodityRule()){
				...
				productInfoApi.updateProductInformation(
						...
						function (results) {
							...
							self.waitingForUpdate = false;
							fetchError('Error updating proudct information', error);
						}
				)
			}else{
				self.error = "Stop! Hierarchy changes requires more review.";
			}
		};

		/**
		 * Check sub commodity rule before save.
		 */
		self.checkSubCommodityRule = function(){
			var returnData = false;
			if(self.currentProductMasterParams.productMaster.subCommodityCode
					!== self.originalProductMasterParams.productMaster.subCommodityCode){//has change data
				if(self.currentSubCommodity.key.classCode === self.currentProductMasterParams.productMaster.classCode){//classCode
					returnData = true;
				}
			}else{//no change data
				returnData = true;
		...
		self.getTaxCategoryDefaultWarningMessage = function(){
			return TAX_CATEGORY_DIFFERENT_THAN_SUB_COMMODITY_MESSAGE_TEMPLATE
				.replace("$nonTaxCategory", self.currentProductMasterParams.productMaster.goodsProduct.retailTaxSwitch ? "" : "Non-");
		};
		...
		self.isFoodStampSameAsSubCommodityFoodStamp = function(){
			if (self.productMaster.subCommodity !== undefined && self.productMaster.subCommodity !== null) {
		...
		self.getRetailTaxWarningMessage = function(){
			...
			if (self.currentProductMasterParams.productMaster.goodsProduct.retailTaxSwitch != self.productMaster.subCommodityCode.taxEligible) {
				if (self.currentProductMasterParams.productMaster.goodsProduct.retailTaxSwitch) {
		...
		self.getUserById = function() {
			...
			userApi.getUserById({userId: self.productMaster.createUid},
				function (results) {
					...
					self.fetchError("Couldn't find createUid", error);
				}
			)}
		};
		...
		self.prePriceNullToZero = function() {
			if (self.currentProductMasterParams.productMaster.goodsProduct.prePrice==null) {

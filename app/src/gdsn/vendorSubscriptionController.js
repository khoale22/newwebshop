		...
		self.fetchData = function(includeCounts, page) {
			if (self.findAll) {
				gdsnApi.queryVendorSubscriptions({
					page: page, pageSize: self.PAGE_SIZE, includeCounts: includeCounts
				}, self.loadData, self.fetchError);
			}
			else {
				if (self.selectionType == self.GLN) {
					gdsnApi.queryVendorSubscriptionsByGln({
						gln: self.searchText, page: page, pageSize: self.PAGE_SIZE, includeCounts: includeCounts
					}, self.loadData, self.fetchError);
				} else {
					gdsnApi.queryVendorSubscriptionsByVendorName({
						vendorName: self.searchText, page: page, pageSize: self.PAGE_SIZE, includeCounts: includeCounts
					}, self.loadData, self.fetchError);
				}
			}
		};

...
public class ProductSearchService {
	...
	private String exportPrimoPick(ProductMaster productMaster) {
		...
		if (productMaster.getPrimoPickProposedDescription() != null) {
			primoPickProposedDescription = StringUtils.trimToEmpty(productMaster.getPrimoPickProposedDescription()).replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT);
		}
		if(productMaster.getPrimoPickDescription() != null) {
			primoPickDescription = StringUtils.trimToEmpty(productMaster.getPrimoPickDescription()).replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT);
		}
		...
		return String.format(EXPORT_FORMAT_PROM_PICK,
				primoPickSwitch,
				distinctiveSwitch,
				primoPickProposedDescription,
				primoPickDescription,
				primoPickStatus,
				goLocalSwitch);
	}
	...
	private String exportServiceCaseAttributes(ProductMaster productMaster, boolean hadPrimoPick) {
		...
		if(hadPrimoPick){
			returnString = String.format(EXPORT_FORMAT_SERVICE_CASE_ATTRIBUTES,
					...
					shelfTagSizeCode,
					tagItemCode,
					englishCustomerFriendlyDescription1,
					englishCustomerFriendlyDescription2,
					serviceCaseStatus,
					proposedDescription,
					approvedDescription,
					tagType);
	...
	private String exportEcommerce(ProductMaster productMaster, List<TargetSystemAttributePriority> targetSystemAttributePriorities) {
		...
		return String.format(EXPORT_FORMAT_ECOMMERCE,
				...
				ebm,
				productBrand,
				(onlineBrand != null ? onlineBrand.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				(onlineDisplayName != null ? onlineDisplayName.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				(onlineSize != null ? onlineSize.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				(onlineRomanceCopy != null ? onlineRomanceCopy.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				(onlineIngredients != null ? onlineIngredients.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				(onlineDirections != null ? onlineDirections.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				(onlineWarnings != null ? onlineWarnings.replace(DOUBLE_QUOTES_FORMAT, ESCAPED_DOUBLE_QUOTES_FORMAT) : StringUtils.EMPTY),
				publishHebcom,
				publishHeb2u,
				hebcomLowestLevelId,
				heb2uLowestLevelId,

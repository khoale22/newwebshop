...
(function () {
	...
	function ProductShelfAttributesController(productInfoApi, vocabularyApi, userApi, $scope, $rootScope,
		 ...
		 */
        /**
		 * Constant for maxlength of Servicecase Sign
          * @type {number}
         */
		const MAXLENGTH_SERVICE_CASE_SIGN = 56;
		...
		self.isMaxLength = function(result){
            return result.trim().length > 56;
        }
		/**
		...
		self.setTagType = function() {
			if (self.productMaster == null || self.tagTypes == null) {
				return;
			}
		...
		self.setServiceCaseTagData = function (results){
			...
			// add or remove maxlength for Servicecase Sign based on length of data
            if(!self.isMaxLength(self.serviceCaseTagData.proposedDescription))
                self.proposedDescriptionMaxLength = MAXLENGTH_SERVICE_CASE_SIGN;
            else
                self.proposedDescriptionMaxLength = null;
            if(!self.isMaxLength(self.serviceCaseTagData.approvedDescription))
                self.approvedDescriptionMaxLength = MAXLENGTH_SERVICE_CASE_SIGN;
            else
                self.approvedDescriptionMaxLength = null;
			self.serviceCaseSignDescription = null;
		...
		self.validateShelfAttribute = function (){
            ...
            if(self.isMaxLength(self.serviceCaseTagData.proposedDescription)
				&& self.serviceCaseTagDataModified.approvedDescription.trim().length > 56) {
                self.error = "Please enter the Approved Servicecase Sign Description within 56 characters";
                isValid = false;
            }
            if(self.isMaxLength(self.serviceCaseTagData.proposedDescription)
				&& self.serviceCaseTagDataModified.proposedDescription.trim().length > 56) {
                self.error = "Please enter the Proposed Servicecase Sign Description within 56 characters";
                isValid = false;
            }
			return isValid;
		}
		...
		self.updateShelfAttribute = function () {
			...
			if (self.hasValueChanged(self.originalProductMaster.goodsProduct.tagType, self.productMaster.goodsProduct.tagType)) {
        ...
        $rootScope.$on('modalResults', function(event, args){
            if(args.result){
            	console.log("su kien o shelf atriibute");
                self.success = null;
                self.error = null;

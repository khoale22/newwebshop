...
(function () {
	...
	function DirectionController($scope, eCommerceViewApi, $sce, permissionsService,$rootScope) {
		...
		this.$onInit = function () {
			...
			self.isEditingDirection = self.isDinnerTonightTab();
			self.findDirections();
		};
		...
		this.$onChanges = function () {
			...
			if (self.isDinnerTonightTab()) {
				self.findDirectionsDinnerTonight();
			} else {
				self.findDirections();
			}
		};
		...
		/**
		 * Find directions information for the Dinner tonight tab.
		 */
		self.findDirectionsDinnerTonight = function () {
			eCommerceViewApi.findDirectionsDinnerTonight({
					scanCodeId: self.productMaster.productPrimaryScanCodeId
				},
				self.directionResponseSuccess,
				self.fetchError);
		...
		self.setEditMode = function(){
			if(!self.isDinnerTonightTab()) {
				...
				self.isEBM = permissionsService.getPermissions('PD_ECOM_04', 'EDIT');
			}else{
				self.isEBM = true;
				self.isEditingDirection = true;
			}

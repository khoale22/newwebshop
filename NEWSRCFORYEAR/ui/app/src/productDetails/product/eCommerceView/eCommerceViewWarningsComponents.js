	...
	function WarningController($scope, eCommerceViewApi, $sce,permissionsService) {
		...
		this.$onChanges = function () {
			...
			if (self.isDinnerTonightTab()) {
				self.findWarningsDinnerTonight();
			} else {
				self.findWarnings();
			}
		};
		...
		/**
		 * Find warnings information for the Dinner tonight tab.
		 */
		self.findWarningsDinnerTonight = function () {
			eCommerceViewApi.findWarningsDinnerTonight({
					scanCodeId: self.productMaster.productPrimaryScanCodeId
				},
				self.warningResponseSuccess,
				self.fetchError);
		...
		self.warningResponseSuccess = function (result) {
			...
			self.isEditingWarning = self.isDinnerTonightTab();
			if (!result) {
		...
		self.setEditMode = function(){
			if (!self.isDinnerTonightTab()) {
				...
				self.isEBM = permissionsService.getPermissions('PD_ECOM_05', 'EDIT');
			}else{
				self.isEBM = true;
				self.isEditingWarning = true;
			}

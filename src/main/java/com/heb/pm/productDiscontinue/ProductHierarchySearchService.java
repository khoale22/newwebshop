	...
	private PageableResult<ProductDiscontinue> searchByBdmWithCounts(String bdmCode, Pageable request,
		...
		switch (statusFilter) {
			...
			case DISCONTINUED:
				...
				break;
			default:
				data = this.productDiscontinueRepositoryWithCount.findDistinctByProductMasterClassCommodityBdmCode(bdmCode, request);
		}
		...
		switch (statusFilter) {
			...
			default:
				data = this.productDiscontinueRepository.
						findDistinctByProductMasterClassCommodityBdmCode(bdmCode, request);
		}

...
public class ServiceCaseStatusPredicateBuilder implements PredicateBuilder {
	...
	public ExistsClause<?> buildPredicate(CriteriaBuilder criteriaBuilder,
		...
		if (serviceCaseStatusEntry.getStringComparator().equals(ProductMarketingClaim.APPROVED)) {
			return new ExistsClause<>(this.getApprovedSubquery(criteriaBuilder, pmRoot, queryBuilder));
		} else {
			return new ExistsClause<>(
					this.getPendingOrRejectedSubquery(criteriaBuilder, pmRoot, queryBuilder, serviceCaseStatusEntry));
		}
	}


	/**
	 * Builds a predicate to search for pending or rejected service case descriptions.
	 *
	 * @param criteriaBuilder Used to construct the various parts of the SQL statement.
	 * @param pmRoot The root from clause of the main query (this will be used to grab the criteria to join the
	 *               sub-query to).
	 ...
	 * @return A subquery that will return products with pending or rejected status.
	 */
	private Subquery<CandidateWorkRequest> getPendingOrRejectedSubquery(CriteriaBuilder criteriaBuilder,
															Root<? extends ProductMaster> pmRoot,
															CriteriaQuery<? extends ProductMaster> queryBuilder,
		...
		// Constrain on either rejected or pending
		criteria[1] = criteriaBuilder.equal(wrRoot.get(CandidateWorkRequest_.status),
				serviceCaseStatusEntry.getStringComparator().equals(ProductMarketingClaim.REJECTED) ?
						CandidateWorkRequest.StatusCode.REJECTED.getName() :
						CandidateWorkRequest.StatusCode.IN_PROGRESS.getName()
		);

...
public class ProductSearchService {
	...
	public HitsBase<?> getSearchHits(ProductSearchCriteria searchCriteria) {
		...
		if (searchCriteria.hasProductSearch()) {
			...
			return Hits.from(searchCriteria.getProductIds(), productIds, "Proudct", "Products");
		}
	...
	private String exportPrimoPick(ProductMaster productMaster) {
		...
		if (productMaster.getPrimoPickProposedDescription() != null) {
			primoPickProposedDescription = StringUtils.trimToEmpty(productMaster.getPrimoPickProposedDescription()).replace("\"", "\"\"");
		}
		if(productMaster.getPrimoPickDescription() != null) {
			primoPickDescription = StringUtils.trimToEmpty(productMaster.getPrimoPickDescription()).replace("\"", "\"\"");
		}
	...
	private String exportEcommerce(ProductMaster productMaster, List<TargetSystemAttributePriority> targetSystemAttributePriorities) {
		...
		return String.format(EXPORT_FORMAT_ECOMMERCE,
				...
				ebm,
				productBrand,
				(onlineBrand != null ? onlineBrand.replace("\"", "\"\"") : StringUtils.EMPTY),
				(onlineDisplayName != null ? onlineDisplayName.replace("\"", "\"\"") : StringUtils.EMPTY),
				(onlineSize != null ? onlineSize.replace("\"", "\"\"") : StringUtils.EMPTY),
				(onlineRomanceCopy != null ? onlineRomanceCopy.replace("\"", "\"\"") : StringUtils.EMPTY),
				(onlineIngredients != null ? onlineIngredients.replace("\"", "\"\"") : StringUtils.EMPTY),
				(onlineDirections != null ? onlineDirections.replace("\"", "\"\"") : StringUtils.EMPTY),
				(onlineWarnings != null ? onlineWarnings.replace("\"", "\"\"") : StringUtils.EMPTY),
				publishHebcom,
				publishHeb2u,
				hebcomLowestLevelId,
				heb2uLowestLevelId,

...
public interface ProductInfoRepository extends JpaRepository<ProductMaster, Long>, ProductInfoRepositoryCommon{
	 ...
	 */
	@Query(value="select productMaster.prodId from ProductMaster  productMaster where exists (select searchCriteria.productId from SearchCriteria searchCriteria where productMaster.prodId = searchCriteria.productId and searchCriteria.sessionId = :sessionId)")

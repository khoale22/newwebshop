...
public class ProductUpdatesTaskPredicateBuilder {
        ...
        return new Specification<AlertStaging>() {
            ...
            public Predicate toPredicate(Root<AlertStaging> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if(showOnSite != null && !showOnSite.trim().isEmpty()) {
                    Subquery<DB2ProductOnline> showOnSiteSubquery = buildShowOnSitePredicate(showOnSite, root, query, cb);
     ...
     */
    private Subquery<DB2ProductOnline> buildShowOnSitePredicate(String showOnSite, Root<AlertStaging> root,
                                                                CriteriaQuery<?> queryBuilder, CriteriaBuilder cb) {
        Subquery<DB2ProductOnline> productOnlineExist = queryBuilder.subquery(DB2ProductOnline.class);
        Root<DB2ProductOnline> poRoot = productOnlineExist.from(DB2ProductOnline.class);
        productOnlineExist.select(poRoot.get("key").get("productId"));

        Predicate[] predicates = new Predicate[5];
        predicates[0] = cb.equal(poRoot.get(DB2ProductOnline_.key).get(DB2ProductOnlineKey_.productId),
                root.get(AlertStaging_.alertKey).as(Long.class));
        predicates[1] = cb.equal(poRoot.get(DB2ProductOnline_.key).get(DB2ProductOnlineKey_.saleChannelCode),
                DB2ProductOnlineKey.SALES_CHANNEL_HEB_COM);
        predicates[2] = cb.equal(poRoot.get(DB2ProductOnline_.showOnSite), true);
        Date today = java.sql.Date.valueOf(LocalDate.now());
        predicates[3] = cb.lessThanOrEqualTo(poRoot.get(DB2ProductOnline_.key).get(DB2ProductOnlineKey_.effectiveDate),today);
        predicates[4] = cb.greaterThanOrEqualTo(poRoot.get(DB2ProductOnline_.expirationDate),today);

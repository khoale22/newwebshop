...
public class ProductManagementServiceClient extends BaseWebServiceClient
	...
	public void updateVendLocImprtItm(ImportItem importItem) {
		...
		vendLocImprtItm.setPRODNMINORDDES(importItem.getMinOrderDescription());
		vendLocImprtItm.setPRODNMINORDQTY(importItem.getMinOrderQuantity().toString());
		vendLocImprtItm.setPCKUPPNTTXT(importItem.getPickupPoint());
		...
		vendLocImprtItm.setSEASNTXT(importItem.getSeason());
		vendLocImprtItm.setSELLYY(importItem.getSellByYear().toString());
		vendLocImprtItm.setCOLORDES(importItem.getColor());
		vendLocImprtItm.setCRTNMRKNGTXT(importItem.getCartonMarking());
		...
		vendLocImprtItm.setDUTYINFOTXT(importItem.getDutyInformation());
	 ...
	 */
	public void updateMrtQuantity(Shipper shipper) {
		...
		//'' = Add , M is required for any type of modification that is not update
		pdShipper.setACTIONCD(UPDATE_ACTION);
	 ...
	 * Save to mrt.
	 *
	 * @param shipper the shipper in an MRT
	 *                Calls webservice response to add a new shipper to an MRT
	 */
	public void saveToMrt(Shipper shipper) {
		...
		//A = Add , M is required for any type of modification that is not update
		pdShipper.setACTIONCD("A");
		pdShipper.setPDSHPRTYPCD("M");
		pdShipper.setWorkId(workId.trim());
		pdShipper.setPDSHPRQTY(shipper.getShipperQuantity().toString().trim());
		pdShipper.setPDSHPRUPCNO(String.valueOf(shipper.getKey().getShipperUpc()));
		pdShipper.setPDUPCNO(String.valueOf(shipper.getKey().getUpc()));
	 ...
	 *
	 * @param shipper the shipper
	 *                Calls webservice response to delete a shipper from an MRT
	 */
	public void deleteFromMrt(Shipper shipper) {
		...
		//D = Delete, M is required for any type of modification that is not update
		pdShipper.setACTIONCD("D");
		pdShipper.setPDSHPRTYPCD("M");
		pdShipper.setWorkId(workId.trim());
		pdShipper.setPDSHPRQTY(shipper.getShipperQuantity().toString().trim());
		pdShipper.setPDSHPRUPCNO(String.valueOf(shipper.getKey().getShipperUpc()));
	...
	public void updateECommerceViewInformation(ECommerceViewDetails eCommerceViewDetails, String userId) {
		...
		if(eCommerceViewDetails.getDisplayName() != null){
			ProdScnCdExtent prodScnCdExtent = this.generateProductScanCodeExtension(eCommerceViewDetails
							.getScanCodeId(), eCommerceViewDetails.getDisplayName().getContent(),
		...
		if(eCommerceViewDetails.getRomanceCopy() != null){
			ProdScnCdExtent prodScnCdExtent = this.generateProductScanCodeExtension(eCommerceViewDetails
							.getScanCodeId(), eCommerceViewDetails.getRomanceCopy().getContent(),

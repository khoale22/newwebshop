...
public class ProductAttributeManagementServiceClient extends BaseWebServiceClient
	...
	private static final String DINNER_TONIGHT_CHANNEL = "07   ";
...
//			}
	...
	public void publishECommerceViewDataToHebCom(ECommerceViewDetails product,  String userId) {
		...
		if(DINNER_TONIGHT_CHANNEL.trim().equals(product.getSalsChnlCd().trim())){
			// For dinner tonight, we just publish nutrient and ingredient.
			request.getCopyeCommerceattributes().add(
					this.createCopyeCommerceAttributes(product.getScanCodeId(), product.getNutrient().getLogicAttributeId(),
							product.getSalsChnlCd(), userId, product.getAlertId()));
			request.getCopyeCommerceattributes().add(this.createCopyeCommerceAttributes(product.getScanCodeId(),
					product.getIngredients().getLogicAttributeId(), product.getSalsChnlCd(), userId, product.getAlertId()));
			request.getCopyeCommerceattributes().add(this.createCopyeCommerceAttributes(product.getScanCodeId(),
					product.getDinnerTonightTags().getLogicAttributeId(), product.getSalsChnlCd(), userId, product.getAlertId()));
			request.getCopyeCommerceattributes().add(this.createCopyeCommerceAttributes(product.getScanCodeId(),
					product.getSpecification().getLogicAttributeId(), product.getSalsChnlCd(), userId, product.getAlertId()));
		}else{
			// Common publish.
	...
	/**
	 * Update master data attribute for DinnerTonight.
	 *
	 * @param product the ECommerceViewDetails object.
	 * @param userId  the user id.
	 */
	public void updateMasterDataExtentAttributeForDinnerTonight(ECommerceViewDetails product, String userId) {
		ProductAttributeManagementServiceClient.logger.info(String.format(BEGIN_UPDATE_ECOMMERCE_VIEW_FOR_SOURCE27_LOG, userId));
		List<MasterDataExtnAttribute> masterDataExtnAttributes = new ArrayList<>();
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getBrand(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getSize(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getDisplayName(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getDinnerTonightProductDescription(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getSecondaryProductDescription(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getDinnerTonightTags(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getIngredients(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getSpecification(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getDirections(), masterDataExtnAttributes);
		addMasterDataExtnAttributeForDinnerTonight(product.getScanCodeId(), userId, product.getWarnings(), masterDataExtnAttributes);
		UpdateProductAttributeRequest request = new UpdateProductAttributeRequest();
		request.setAuthentication(this.getAuthentication());
		request.setTrackingNbr(String.valueOf(0));
		request.getMasterDataExtnAttribute().addAll(masterDataExtnAttributes);
		if (product.getIngredients() != null) {
			addProductAttributeOverrideForDinnerTonight(product.getProductId(), product.getIngredients(), request);
		}
		// Update Panel type.
		List<ProdPackVariation> prodPkVariations = this.createProdPkVariation(product.getScanCodeId(), product.getNutriPanelTypCd(), userId);
		if (!prodPkVariations.isEmpty()) {
			request.getProdPkVariation().addAll(prodPkVariations);
		}
		// Update master data extent attribute.
		this.updateProductAttribute(request);
		// update Show On Site Date
		this.updateShowOnSiteDate(product);
		// update Fulfillment Program
		this.productManagementServiceClient.updateFulfillmentProgram(product.getProductFullfilmentChanels(), product.getProductId(), userId);
	...
	/**
	 * Create MasterDataExtnAttribute for dinner to night.
	 *
	 * @param upc       the scan code.
	 * @param userId    the user id.
	 * @param attribute the ECommerceViewAttributePriorities object.
	 * @return the MasterDataExtnAttribute object.
	 */
	private void addMasterDataExtnAttributeForDinnerTonight(Long upc, String userId, ECommerceViewAttributePriorities attribute, List<MasterDataExtnAttribute> masterDataExtnAttributes) {
		if (attribute != null) {
			long sourceSystemId = ProductECommerceViewService.SourceSystemNumber.DINNER_TONIGHT_SOURCE_SYSTEM_NUMBER.getValue();
			long attributeId;
			if (ProductECommerceViewService.LogicAttributeCode.INGREDIENT_STATEMENT_LOGIC_ATTRIBUTE_ID.getValue()
					.equals(attribute.getLogicAttributeId())) {
				attributeId = attribute.getPhysicalAttributeId();
			} else {
				attributeId = attribute.getLogicAttributeId();
			}
			...
			masterDataExtnAttribute.setATTRID(String.valueOf(attributeId));
			masterDataExtnAttribute.setKEYID(String.valueOf(upc));
			masterDataExtnAttribute.setITMPRODKEYCD(UPC_ITEM_KEY_CODE.trim());
			masterDataExtnAttribute.setATTRVALTXTTEXT(attribute.getContent().trim());
			masterDataExtnAttribute.setACTIONCD(this.getActionCodeForMasterDataExtnAttribute(attributeId, upc, sourceSystemId));
			masterDataExtnAttribute.setDTASRCSYS(String.valueOf(sourceSystemId));
			...
			masterDataExtnAttribute.setLSTUPDTTS(LocalDate.now().toString());
			masterDataExtnAttributes.add(masterDataExtnAttribute);
		}
	}

	/**
	 * Add product attribute override for ingredient in dinner tonight tab.
	 * Check if record exist in DB then delete it, and add a new record.
	 *
	 * @param productId the product id to update.
	 * @param attributePriority the ECommerce view attribute priorities contains ingredient to update.
	 * @param request the request that adds product attribute override.
	 */
	private void addProductAttributeOverrideForDinnerTonight(Long productId,
															 ECommerceViewAttributePriorities attributePriority,
															 UpdateProductAttributeRequest request) {
		String psWorkId = this.getWorkId().toString();
		ProductAttributeOverwrite productAttribute = this.productAttributeOverwriteRepository
				.findByKeyItemProductKeyCodeAndKeyKeyIdAndKeyLogicAttributeId(
						ProductAttributeManagementServiceClient.PRODUCT_ITEM_KEY_CODE, productId,
						attributePriority.getLogicAttributeId());
		boolean isAddNew = false;
		if (productAttribute != null && !ProductECommerceViewService.SourceSystemNumber.DINNER_TONIGHT_SOURCE_SYSTEM_NUMBER
				.getValue().equals(productAttribute.getKey().getSourceSystemId())) {
			this.createProductAttributeOverride(productAttribute.getKey().getLogicAttributeId(),
					productAttribute.getKey().getPhysicalAttributeId(), productId, psWorkId,
					ACTION_CODE.ACTION_CD_DELETE.getValue(), productAttribute.getKey().getSourceSystemId(), request);
			isAddNew = true;
		}
		if (isAddNew) {
			this.createProductAttributeOverride(attributePriority.getLogicAttributeId(),
					attributePriority.getPhysicalAttributeId(), productId, psWorkId, ACTION_CODE.ACTION_CD_ADD.getValue(),
					ProductECommerceViewService.SourceSystemNumber.DINNER_TONIGHT_SOURCE_SYSTEM_NUMBER.getValue(), request);
		}
	}
	 ...
	 * Update show on site date.
	 *
	 * @param showOnSiteDate the sub date
	 * @param showOnSiteDate the sub date org
	 * @return the string
	 * @author hanh.tran
	 */
	private void updateShowOnSiteDate(ECommerceViewDetails showOnSiteDate) {
		if (showOnSiteDate.getShowOnSiteStartDate() != null && showOnSiteDate.getShowOnSiteEndDate() != null) {
			ProductOnline productOnlineOrg = this.productOnlineRepository
					.findTop1ByKeyProductIdAndKeySaleChannelCodeAndShowOnSiteOrderByKeyEffectiveDateDesc(showOnSiteDate.getProductId(),
							DINNER_TONIGHT_CHANNEL, ProductECommerceViewService.PRODUCT_SHOW_ON_SITE);
			UpdateProductAttributeRequest request = new UpdateProductAttributeRequest();
			request.setAuthentication(this.getAuthentication());
			request.setTrackingNbr(String.valueOf(0));
			...
			if (productOnlineOrg != null) {
				...
				productOnline.setPRODID(String.valueOf(showOnSiteDate.getProductId()));
				...
				productOnline.setSALSCHNLCD(DINNER_TONIGHT_CHANNEL.trim());
				request.getProductOnline().add(productOnline);
			}
			//add new show on site
			if(showOnSiteDate.isShowOnSite()) {
				com.heb.xmlns.ei.productonline.ProductOnline productOnline = new com.heb.xmlns.ei.productonline.ProductOnline();
				productOnline.setWORKID(String.valueOf(0));
				productOnline.setPRODID(String.valueOf(showOnSiteDate.getProductId()));
				...
				productOnline.setEFFDT(ProductECommerceViewUtil.convertDateToStringDateYYYYMMDD(showOnSiteDate.getShowOnSiteStartDate()));
				productOnline.setEXPRNDT(ProductECommerceViewUtil.convertDateToStringDateYYYYMMDD(showOnSiteDate.getShowOnSiteEndDate()));
				request.getProductOnline().add(productOnline);
			}
			//call request
			if(request.getProductOnline() != null && !request.getProductOnline().isEmpty()) {
				this.updateProductAttribute(request);
			}
		}
	}
			...
			if(eCommerceViewAttributePriorityDetails.isSelected()){
				...
				if(eCommerceViewAttributePriorityDetails.isSourceEditable()){
					if(eCommerceViewAttributePriority.getAttributeId().equals(ProductECommerceViewService
						...
						this.productManagementServiceClient.updateDataSourceForProdScnCdExtent
								(eCommerceViewAttributePriority.getProductId(), eCommerceViewAttributePriority
												.getPrimaryScanCode(), eCommerceViewAttributePriorityDetails.<String>getContent()
												.getContent(), ProductManagementServiceClient.PROD_EXT_DTA_CD_FOR_ECOM_DES1,
										userId);
						...
						this.productManagementServiceClient.updateDataSourceForProdScnCdExtent
								(eCommerceViewAttributePriority.getProductId(), eCommerceViewAttributePriority
												.getPrimaryScanCode(), eCommerceViewAttributePriorityDetails.<String>getContent()
												.getContent(), ProductManagementServiceClient
												.PROD_EXT_DTA_CD_FOR_ECOM_DES2,
										userId);
					}
					else {
						MasterDataExtnAttribute masterDataExtnAttribute = this.createMasterDataExtensionAttribute
								(eCommerceViewAttributePriorityDetails, eCommerceViewAttributePriority.getPrimaryScanCode
										(), userId);
		...
		for (ProductAttribute productAttribute : productAttributes){
			for(ProductAttributeValue productAttributeValue : productAttribute.getValues()){
				...
				attribute.setATTRVALTS(
						...
						productAttributeValue.getTimestamp().toString() : StringUtils.EMPTY);

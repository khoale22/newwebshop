...
public class ShelfAttributesService {
		...
		if (productMaster.getPrimoPickProperties() != null) {
			...
			if (productMaster.getPrimoPickProperties().getPrimoPickStatus() != null) {
				...
				if (productMaster.getPrimoPickProperties().getPrimoPickStatus().equals(ProductMarketingClaim.SUBMITTED.trim())) {
					if (!user.canUserEditResource(ResourceConstants.SHELF_ATTRIBUTES_SUBMIT_PRIMO_PICK)) {
					...
					productMarketingClaims.add(constructProductMarketingClaim(productMaster.getProdId(),
							...
							MarketingClaim.Codes.PRIMO_PICK.getCode(), null, null));
				}
				...
				if (productMaster.getPrimoPickProperties().getPrimoPickStatus().equals(ProductMarketingClaim.APPROVED.trim())) {
					if (!user.canUserEditResource(ResourceConstants.SHELF_ATTRIBUTES_APPROVE_PRIMO_PICK)) {
					...
					// It must be distinctive when approving primo pick or update Expiration date to future
	...
	private boolean hasApprovedServiceCaseSignChanged(Long productId, String serviceCaseSignDescription) {
		...
		if (approvedDescription!= null) {
			return !approvedDescription.equals(serviceCaseSignDescription);
		}
		return serviceCaseSignDescription != null;
	}
    ...
    public List<ProductDescription> updateServiceCaseTagData(ServiceCaseSign serviceCaseSign, UserInfo user){
		...
		if (serviceCaseSign.getStatus() == null) {
			serviceCaseSign.setStatus(CandidateWorkRequest.REQUEST_STATUS_WORKING);
		}
		...
		if (approvedDescriptionChanged && !serviceCaseSign.getStatus().equals(CandidateWorkRequest.REQUEST_STATUS_PASS)) {
			throw new IllegalArgumentException("Approved service case descriptions can only be changed for approved products");
		}

		long statusChange = 0;

		// Validate the user has the right permissions.
		if (serviceCaseSign.getStatus().equals(CandidateWorkRequest.REQUEST_STATUS_WORKING)) {
			...
			if (!user.canUserEditResource(ResourceConstants.SHELF_ATTRIBUTES_SUBMIT_SERVICE_CASE_SIGN)) {
				throw new IllegalArgumentException("User does not have access to edit proposed service case sign description");
			}
		}
		if (serviceCaseSign.getStatus().equals(CandidateWorkRequest.REQUEST_STATUS_PASS)) {
			...
			// if the reqeust is approved, an approved description is required.
			if (serviceCaseSign.getApprovedDescription() == null || serviceCaseSign.getApprovedDescription().isEmpty()) {
				throw new IllegalArgumentException("A description is required for approved  service case sign descriptions");
			}
			if (!user.canUserEditResource(ResourceConstants.SHELF_ATTRIBUTES_APPROVE_CASE_SIGN)) {
				throw new IllegalArgumentException("User does not have access to approve service case sign description");
			}
		}
		if (serviceCaseSign.getStatus().equals(CandidateWorkRequest.REQUEST_STATUS_REJECTED)) {
			...
			if (!user.canUserEditResource(ResourceConstants.SHELF_ATTRIBUTES_APPROVE_CASE_SIGN)) {
				throw new IllegalArgumentException("User does not have access to reject service case sign description");
			}
		}

...
import com.heb.pm.customHierarchy.HierarchyContextService;
...
public class ProductTaxonomyAttributeService {
	...
	private static final String CASE_PACK = "CASEPACK";
	...
	public List<ProductAttribute> getByProductIdAndProductType(Long productId, String productType) {
		...
		attributes = this.filterAttributesByProductTypes(
				attributes,
				productType);
		return this.getProductAttributesByProductIdAndProductType(
				productMaster,
				productType,
				parentRelationships,
				attributes);
	...
	private Set<Attribute> getAllAttributesForProduct(List<GenericEntityRelationship> parentRelationships) {
		...
		this.addAllAttributesFromParents(parentRelationships, toReturn);
		return toReturn;
	}
	 ...
	 * @param attributes The attributes.
	 */
	private void addAllAttributesFromParents(List<GenericEntityRelationship> parentRelationships, Set<Attribute> attributes) {
		...
		parentRelationships.forEach(relationship -> {
			relationship.getGenericParentEntity().getEntityAttributes().forEach(
					entityAttribute -> attributes.add(entityAttribute.getAttribute()));
			this.addAllAttributesFromParents(relationship.getParentRelationships(), attributes);
		});
	}
			...
			for (ProductAttributeValue value : productAttribute.getValues()) {
				if (!this.findMatchingProductAttributeValue(value, compareProductAttribute)) {
					if("D".equals(action)) {
						...
						values.add(
								new ProductAttributeValue(value.getType(), value.getText(), value.getNumber(), value.getTimestamp(),
										value.getDate(), value.getCode(), ++nextSequence, SourceSystem.SourceSystemNumber.SOURCE_SYSTEM_PRODUCT_MAINTENANCE.getValue(), action)
						);
					}
				}
			}

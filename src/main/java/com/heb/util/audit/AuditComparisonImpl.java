...
public class AuditComparisonImpl implements AuditComparisonDelegate {
	...
	private AuditRecord processField(Field field, Audit currentAudit, Audit previousAudit, String userDisplayName, String...filters)
		...
		boolean isAuditForEcommerce = false;
		for (String filter: filters) {
			isAuditForEcommerce = FilterConstants.ECOMMERCE_VIEW_AUDIT.contains(filter);
		}
		if(filters.length>0){
			for (String filter: filters) {
		...
		try {
			...
			if(!isEquals){
				...
				} else if (currentAudit instanceof MasterDataExtensionAudit && isAuditForEcommerce) {
					MasterDataExtensionAudit auditImpl = (MasterDataExtensionAudit) currentAudit;
					attrName = auditImpl.getMasterDataExtensionAuditDisplayName();
				} else if (currentAudit instanceof ProductScanCodeExtentAudit && isAuditForEcommerce) {
					ProductScanCodeExtentAudit auditImpl = (ProductScanCodeExtentAudit) currentAudit;
					attrName = auditImpl.getProductScanCodeExtentAuditDisplayName();
				}
				else {
